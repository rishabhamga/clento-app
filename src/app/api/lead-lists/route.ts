...
export interface SyndieLeadList {
    id: string,
    name: string,
    listType: string,
...
const getLeadLists = async (tokenData: { api_token: string }, page: number, limit: number) => {
    const res = await axios.get(syndieBaseUrl + '/api/lead-lists',
        {
            params: {
                page,
                limit: limit
    ...
    return res.data as LeadListsResponse;
}
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!orgId) {
            console.log('❌ Lead Lists API: Unauthorized - no orgId')
            return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        }
        ...
        if (orgData && !orgError) {
            ...
            if (tokenData && !tokenError) {
                const leadLists = await getLeadLists(tokenData, 1, 50);
                return NextResponse.json({ listType: 'syndie', data: leadLists }, { status: 200 });
            }
        }
