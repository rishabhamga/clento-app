-- Enhanced Leads Schema Migration
-- Add all missing Apollo API fields to store rich lead data
...
-- Add professional categorization fields
ALTER TABLE leads 
ADD COLUMN IF NOT EXISTS departments JSONB DEFAULT '[]'::jsonb,
ADD COLUMN IF NOT EXISTS subdepartments JSONB DEFAULT '[]'::jsonb,
ADD COLUMN IF NOT EXISTS seniority VARCHAR(255),
ADD COLUMN IF NOT EXISTS functions JSONB DEFAULT '[]'::jsonb,
ADD COLUMN IF NOT EXISTS years_experience INTEGER,
ADD COLUMN IF NOT EXISTS time_in_current_role VARCHAR(255);
...
ADD COLUMN IF NOT EXISTS company_linkedin VARCHAR(255),
ADD COLUMN IF NOT EXISTS company_founded_year INTEGER,
ADD COLUMN IF NOT EXISTS company_logo_url TEXT,
ADD COLUMN IF NOT EXISTS company_phone VARCHAR(50),
ADD COLUMN IF NOT EXISTS company_alexa_ranking INTEGER,
ADD COLUMN IF NOT EXISTS company_primary_domain VARCHAR(255);

-- Add company growth metrics
ALTER TABLE leads 
ADD COLUMN IF NOT EXISTS company_headcount_six_month_growth DECIMAL(5,4),
ADD COLUMN IF NOT EXISTS company_headcount_twelve_month_growth DECIMAL(5,4),
ADD COLUMN IF NOT EXISTS company_headcount_twenty_four_month_growth DECIMAL(5,4);
...
CREATE INDEX IF NOT EXISTS idx_leads_company_founded_year ON leads(company_founded_year);
CREATE INDEX IF NOT EXISTS idx_leads_email_status ON leads(email_status);

-- Create GIN indexes for JSONB fields
CREATE INDEX IF NOT EXISTS idx_leads_departments_gin ON leads USING GIN(departments);
CREATE INDEX IF NOT EXISTS idx_leads_subdepartments_gin ON leads USING GIN(subdepartments);
CREATE INDEX IF NOT EXISTS idx_leads_functions_gin ON leads USING GIN(functions);
CREATE INDEX IF NOT EXISTS idx_leads_employment_history_gin ON leads USING GIN(employment_history);
CREATE INDEX IF NOT EXISTS idx_leads_keywords_gin ON leads USING GIN(keywords);
...
BEGIN
  ...
  INSERT INTO leads (
    user_id, external_id, first_name, last_name, full_name, email, phone, headline, photo_url,
    ...
    current_user_id, p_external_id, p_first_name, p_last_name, p_full_name, p_email, p_phone, p_headline, p_photo_url,
